...
function HomePageContent() {
  ...
  // State quản lý ẩn hiện menu 3 gạch trên Mobile và Dropdown trên PC
  ...
  const handleRoutingFilters = async () => {
    setLoading(true);
    setActiveMenu(null);
    setIsMobileMenuOpen(false); // Tự động đóng menu sau khi bấm chọn danh mục
    let apiUrl = "";
    let pageTitle = "";
  ...
  return (
    <div className="app-main-wrapper" style={{ backgroundColor: "#060606", color: "#cccccc", fontFamily: "'Inter', sans-serif", minHeight: "100vh", fontSize: "14px", WebkitFontSmoothing: "antialiased" }}>
      ...
      <header className="main-header" style={{ backgroundColor: "#000000", padding: "10px 50px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "fixed", top: 0, left: 0, right: 0, zIndex: 9999, borderBottom: "1px solid #1a1525" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "30px" }}>
          ...
          {/* NAV MENU: PC thì hiển thị ngang, Mobile biến thành menu dọc ẩn/hiện */}
          <nav className={`nav-menu ${isMobileMenuOpen ? "mobile-open" : ""}`} style={{ display: "flex", gap: "20px", fontSize: "13px", fontWeight: "600", color: "#b3b3b3" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {(activeMenu === "genre" || isMobileMenuOpen) && (
                <div className="dropdown-sub-menu" style={{ position: "absolute", top: "25px", left: 0, backgroundColor: "#0f0f0f", border: "1px solid #222", padding: "10px", borderRadius: "4px", width: "160px", display: "grid", gridTemplateColumns: "1fr", gap: "8px", zIndex: 110, boxShadow: "0 10px 25px rgba(0,0,0,0.7)" }}>
                  {genres.map((g) => (
                    <span key={g._id} onClick={() => router.push(`/?genre=${g.slug}`)} style={{ cursor: "pointer", color: genreParam === g.slug ? "#8a3ffc" : "#cccccc", fontSize: "13px" }}>{g.name}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div style={{ position: "relative" }}>
              ...
              {(activeMenu === "country" || isMobileMenuOpen) && (
                <div className="dropdown-sub-menu" style={{ position: "absolute", top: "25px", left: 0, backgroundColor: "#0f0f0f", border: "1px solid #222", padding: "10px", borderRadius: "4px", width: "160px", display: "grid", gridTemplateColumns: "1fr", gap: "8px", zIndex: 110, boxShadow: "0 10px 25px rgba(0,0,0,0.7)" }}>
                  {countries.map((c) => (
                    <span key={c._id} onClick={() => router.push(`/?country=${c.slug}`)} style={{ cursor: "pointer", color: countryParam === c.slug ? "#8a3ffc" : "#cccccc", fontSize: "13px" }}>{c.name}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      {/* CONTAINER CHÍNH */}
      <div className="main-content-layout" style={{ maxWidth: "1280px", margin: "0 auto", padding: "95px 20px 25px 20px" }}>
                  ...
                  {searchMovies.length > 0 ? (
                    searchMovies.map((movie) => (
                      <div key={movie._id} onClick={() => router.push(`/movie/${movie.slug}`)} style={{ cursor: "pointer", marginBottom: "15px" }}>
                        <div className="category-movie-card" style={{ position: "relative", width: "100%", height: "240px", borderRadius: "6px", overflow: "hidden", backgroundColor: "#11", boxShadow: "0 4px 10px rgba(0,0,0,0.5)" }}>
                          <img src={getCleanImageUrl(movie.poster_url || movie.thumb_url)} alt={movie.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
            ...
            <div style={{ backgroundColor: "#0b0b0b", borderRadius: "6px", border: "1px solid #141414", overflow: "hidden", marginBottom: "35px" }}>
              {moviesCinema.slice(0, 7).map((movie, index) => (
                <div key={movie._id} onClick={() => router.push(`/movie/${movie.slug}`)} style={{ display: "flex", alignItems: "center", padding: "12px 15px", borderBottom: index === 6 ? "none" : "1px solid #131313", cursor: "pointer" }}>
                  ...
                  <div style={{ width: "35px", height: "45px", borderRadius: "4px", overflow: "hidden", marginRight: "12px", backgroundColor: "#11", flexShrink: 0 }}>
                    <img src={getCleanImageUrl(movie.thumb_url || movie.poster_url)} alt={movie.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
                  </div>
